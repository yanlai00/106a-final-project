...
def cross_product(w, q):
    return np.dot(kfs.skew_3d(w), q)
...
def baxter_forward_kinematics_from_angles(joint_angles):
    ...
    for i in range(7):
        xi_array[:3, i] = - cross_product(ws[0:3,i], qs[0:3,i])
        xi_array[3:, i] = ws[0:3,i]
...
def baxter_forward_kinematics_from_joint_state(joint_state):
    ...
    new_angles[:2] = angles[2:4]
    new_angles[2:4] = angles[:2]
    new_angles[4:] = angles[4:]
    # print(baxter_forward_kinematics_from_angles(new_angles))
    return baxter_forward_kinematics_from_angles(new_angles)