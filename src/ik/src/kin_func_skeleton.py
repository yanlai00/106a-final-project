...
def rotation_3d(omega, theta):
    ...
    rot = np.eye(3) + omega_hat / omega_norm * np.sin(omega_norm * theta) + np.matmul(omega_hat, omega_hat) / (omega_norm * omega_norm) * (1 - np.cos(omega_norm * theta))
...
def homog_3d(xi, theta):
    ...
    if np.any(w):
        ...
        xi_hat[:3, :3] = rot
        w_reshape = w.reshape((3, 1))
        xi_hat[:3, 3:] = (np.matmul((np.eye(3) - rot), np.matmul(skew_3d(w), v)) + np.matmul(np.matmul(w_reshape, w_reshape.T), v) * theta) / (np.linalg.norm(w) * np.linalg.norm(w))
        xi_hat[3, 3] = 1
    else:
        xi_hat[:3, :3] = np.eye(3)
        xi_hat[:3, 3:] = v * theta
...
def prod_exp(xi, theta):
    ...
    for i in range(xi.shape[1]):
        xi_i = xi[:, i]
        theta_i = theta[i]
        prod = np.matmul(prod, homog_3d(xi_i, theta_i))
    return prod
